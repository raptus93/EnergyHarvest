...
public class RegistryActivity extends Activity {
	...
	public void confirm(View view) {
		...
		if(username.length() == 0 || email.length() == 0 || password.length() == 0 || passwordRepeat.length() == 0) {
			Toast.makeText(RegistryActivity.this, "Angaben unvollst�ndig!", Toast.LENGTH_SHORT).show();
		}
		else {
			if(!password.equals(passwordRepeat)) {
				Toast.makeText(RegistryActivity.this, "Passwort stimmt nicht �berein!", Toast.LENGTH_SHORT).show();
			}
			else {
